...
|---|---:|---|---|
| Steganography | 3/23 | Задачи на поиск скрытых данных в изображениях, файлах и метаданных | [Открыть](./steganography/) |
...
| Наименее значимый мем | Steganography | ![Easy](https://img.shields.io/badge/Easy-Green?style=for-the-badge) | LSB-стеганография в PNG: `zsteg` находит флаг в младшем бите RGB (`b1,rgb,lsb`) | [Открыть](./steganography/leastsignificantmeme/) |
...
| Простой калькулятор | PWN | ![Easy](https://img.shields.io/badge/Easy-Green?style=for-the-badge) | Произвольная запись (индекс без нижней границы) → перезапись `puts@got` на `system@plt`, затем `greet("/bin/sh")` | [Открыть](./pwn/Simplecalculator/) |
...
| Сверхсекретный Шпион | Forensics | ![Easy](https://img.shields.io/badge/Easy-Green?style=for-the-badge) | Скрытый канал в UDP-портах: `srcport−30000` = индекс, символ = `32768−dstport` — собираем флаг из заголовков | [Открыть](./forensics/TopSecretSpy/) |
| Новогодняя открытка | Forensics | ![Easy](https://img.shields.io/badge/Easy-Green?style=for-the-badge) | К JPEG дописан WAV-трек; флаг в спектрограмме аудио, виден только при zoom по времени (`sox trim`) | [Открыть](./forensics/NewYearscard/) |
| Офисный Хакер | Forensics | ![Easy](https://img.shields.io/badge/Easy-Green?style=for-the-badge) | Скрытый канал в трейлере кадра (SLL trailer) под прикрытием IPP-печати; сбор флага через `tshark -e sll.trailer` | [Открыть](./forensics/OfficeHacker/) |
| Синий иней | Forensics | ![Easy](https://img.shields.io/badge/Easy-Green?style=for-the-badge) | Bluetooth-дамп: под аудио скрыта OBEX-передача файла; тело = коды через `EF BB BF`, снимаем XOR 30 | [Открыть](./forensics/BlueFrost/) |
